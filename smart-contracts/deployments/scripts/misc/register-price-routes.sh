...
#   cd smart-contracts
#   RPC_URL=... DEPLOYER_PRIVATE_KEY=... ./deployments/scripts/misc/register-price-routes.sh
#
# Required env:
#   - RPC_URL              - RPC endpoint of the target chain (eg. relay devnet)
#   - DEPLOYER_PRIVATE_KEY - must be the RelayPriceOracle owner
#   - MAX_FUTURE_SECONDS   - provider future-time allowance
#
# Optional overrides (defaults are the relay devnet deployment, see
# deployments/contracts/dev.json, and the testnet Chainlink Data Streams
...
cast send "$ORACLE" \
  "setPriceFeedAdapter(bytes32,address,uint32)" \
  "$PROVIDER_CHAINLINK" "$ADAPTER" "$MAX_FUTURE_SECONDS" \
  --rpc-url "$RPC_URL" --private-key "$DEPLOYER_PRIVATE_KEY"
