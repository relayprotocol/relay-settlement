 ...
 *      - Routed multicall router allowlist entry, when ROUTER is set
 *
 * Required env vars:
 ...
 *   ROUTER              Allowlist this multicall router for (CHAIN_ID, DEPOSITORY)
 ...
 *
 * Set ROUTER only once PAYLOAD_BUILDER is the registered builder — the previous
 * builder ignores routed data and would withdraw to the router as a bare transfer.
 */
...
function parseEnv(): Env {
  ...
  return {
    ...
    router: process.env.ROUTER
      ? asAddress(process.env.ROUTER, "ROUTER")
      : undefined,
...
async function main() {
  ...
  console.log(`  router:             ${env.router ?? "unchanged"}`)
  ...
  if (!env.skipConfig) {
    ...
    if (env.router !== undefined) {
      // Read back rather than trusting the tx above: allowlisting a router while an
      // older builder is registered turns routed withdrawals into bare transfers.
      const registered = (await publicClient.readContract({
        address: env.allocator,
        abi: allocatorAbi,
        functionName: "payloadBuilders",
        args: [env.chainId, depository],
      })) as Address
      if (registered.toLowerCase() !== env.payloadBuilder.toLowerCase()) {
        const message = `refusing to allowlist ROUTER: registered builder is ${registered}, not ${env.payloadBuilder}`
        if (!env.dryRun) {
          throw new Error(message)
        }
        console.log(`    [warn] ${message} (continuing: DRY_RUN)`)
      }
      const routerKey = await publicClient.readContract({
        address: env.payloadBuilder,
        abi: ethereumPayloadBuilderAbi,
        functionName: "getRouterAllowedKey",
        args: [
          env.chainId,
          asAddress(env.depository, "DEPOSITORY"),
          env.router,
        ],
      })
      // The builder treats any value other than 1 as disabled; revoking a router
      // means overwriting this key with 0 (Config has no unset), which this script
      // does not generate.
      const routerValue = uint256Word(1n)
      keys.push(routerKey)
      values.push(routerValue)
      console.log(`    routerAllowed    key=${routerKey} value=${routerValue}`)
    } else {
      console.log("    routerAllowed    unchanged (ROUTER not set)")
    }
