#!/usr/bin/env ts-node
/**
 * Configure TronVmPayloadBuilder on the RelayAllocator and Config store.
 *
 * Actions:
 *   1. allocator.setPayloadBuilder(CHAIN_ID, SDK-encoded depository, PAYLOAD_BUILDER)
 *   2. Config.setConfigValues(...) for:
 *      - Tron VM EIP-712 signature chain id
 *      - Tron VM request expiration delay, when EXPIRATION_SECONDS is set
 *
 * Required env vars:
 ...
 *   PAYLOAD_BUILDER     TronVmPayloadBuilder contract address
 *   CHAIN_ID            Relay chain id string
 *   DEPOSITORY          Tron depository address (SDK-encoded by this script)
 *   SIGNATURE_CHAIN_ID  EIP-712 domain chain id for this Tron VM chain
 *
 * Optional env vars:
...
const tronPayloadBuilderAbi = parseAbi([
...
async function main() {
  ...
  const depository = asHexBytes(
    bytesToHex(encodeAddress(env.depository, "tron-vm")),
  ...
  console.log("Configuring Tron VM payload builder")
  ...
  if (!env.skipConfig) {
    console.log("==> Resolving Tron VM config keys")
