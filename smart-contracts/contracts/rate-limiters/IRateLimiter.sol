...
/// @notice Pluggable rate limiter consulted on every FAST_MINT. The oracle passes opaque `data` that
///         the limiter decodes itself, so a new limiter type needs no RelayOracleV2 / SDK change — it
///         is deployed, added to the oracle's limiter allowlist, and fed its own `data`.
interface IRateLimiter {
  ...
  function consume(bytes calldata data) external returns (bool consumed);
}
