...
/// @title TronVmPayloadBuilder
/// @author Relay Protocol
/// @notice Payload builder for "tron-vm" chains
contract TronVmPayloadBuilder is IPayloadBuilder {
  ...
  /// @notice Thrown when an encoded Tron address does not start with the expected `0x41` prefix
  /// @param prefix Actual prefix byte
  error InvalidTronAddressPrefix(bytes1 prefix);

  /// @notice Length of a Tron address, comprised of the `0x41` prefix byte followed by a 20-byte EVM address
  uint256 internal constant TRON_ADDRESS_LENGTH = 21;

  /// @notice Prefix byte that every Tron address starts with
  bytes1 internal constant TRON_ADDRESS_PREFIX = 0x41;

  /// @notice Config contract used to resolve namespaced chain metadata
  Config public immutable CONFIG;

  /// @notice Prefix used when deriving config keys for Tron VM chain id lookups
  bytes32 internal constant TRON_VM_CHAIN_ID_PREFIX =
    keccak256("TRON_VM_CHAIN_ID");

  /// @notice Prefix used when deriving config keys for Tron VM request expiration lookups
  bytes32 internal constant TRON_VM_EXPIRATION_PREFIX =
  ...
  /// @notice Creates a new Tron VM payload builder
  ...
  function buildPayload(
    ...
    if (params.receiver.length != TRON_ADDRESS_LENGTH) {
      revert InvalidReceiverLength(params.receiver.length);
    }
    if (params.currency.length != TRON_ADDRESS_LENGTH) {
    ...
    address currencyAddress = tronAddressToEvmAddress(params.currency);
    address receiverAddress = tronAddressToEvmAddress(params.receiver);
  ...
  function hashesToSign(
    ...
    if (depository.length != TRON_ADDRESS_LENGTH) {
    ...
    bytes32 domainSeparator = Utils.buildDomainSeparator(
      ...
      SIGNATURE_VERSION,
      evmChainId,
      tronAddressToEvmAddress(depository)
    );
  ...
  function getEvmChainIdKey(
    ...
    return keccak256(abi.encodePacked(TRON_VM_CHAIN_ID_PREFIX, chainId));
  }
  ...
  function getExpirationKey() public pure returns (bytes32 key) {
    return TRON_VM_EXPIRATION_PREFIX;
  }

  /// @notice Converts a 21-byte Tron address into its 20-byte EVM `address` representation
  /// @dev Validates the leading `0x41` prefix byte and strips it before decoding
  /// @param tronAddress Encoded Tron address (`0x41` prefix followed by a 20-byte EVM address)
  /// @return evmAddress The underlying EVM address
  function tronAddressToEvmAddress(
    bytes calldata tronAddress
  ) internal pure returns (address evmAddress) {
    if (tronAddress[0] != TRON_ADDRESS_PREFIX) {
      revert InvalidTronAddressPrefix(tronAddress[0]);
    }
    return address(bytes20(tronAddress[1:]));
  }
