...
contract TonVmPayloadBuilder is IPayloadBuilder {
  ...
  error UnsupportedCurrency(uint256 length);
  ...
  ///      bytes must be empty (native TON only). Amount must fit in
  ///      VarUInteger16 (2^120 - 1).
  function buildPayload(
    ...
    if (params.currency.length != 0) {
      revert UnsupportedCurrency(params.currency.length);
    }
