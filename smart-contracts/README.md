...
Solidity coverage is generated by `solidity-coverage` (Hardhat).

```sh
# produce coverage/lcov.info
yarn coverage

# compare current coverage against a baseline lcov file; exits non-zero on regression
yarn coverage:diff path/to/baseline.lcov
```

`yarn coverage:diff` reports any `(file, line)` or branch hit in the baseline that is no longer hit in the current run.
...
When intentionally regenerating a baseline (for example after removing dead contract code), include the literal token `[baseline:refresh]` in the HEAD commit message. The diff script honors this tag and skips the gate so the baseline can be updated in the same PR.

Foundry coverage tooling is wired via `foundry.toml` and `yarn forge:coverage`. There are no `.t.sol` tests yet; tests will land under `test-foundry/`.
