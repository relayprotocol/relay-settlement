...
export function registerSolanaProgramUpgrade(program: Command) {
  program
    .command("solana-program-upgrade")
    .description(
      "Deploy an initial Solana program, upgrade it, and execute a migration function."
    )
    .requiredOption("--rpc <url>", "Solana RPC endpoint")
    .requiredOption(
      "--initial-program-path <path>",
      "Path to the initial .so program file"
    )
    .requiredOption(
      "--upgrade-program-path <path>",
      "Path to the upgrade .so program file"
    )
    .option(
      "--buffer-authority <secret>",
      "Base58 encoded private key of buffer authority (ENV: SOLANA_BUFFER_AUTHORITY)"
    )
    .option(
      "--upgrade-authority <secret>",
    ...
    .action(
      async ({
        ...
        if (!fs.existsSync(initialProgramPath)) {
          throw new Error(
            `Initial program file not found: ${initialProgramPath}`
          )
        }
        if (!fs.existsSync(upgradeProgramPath)) {
          throw new Error(
            `Upgrade program file not found: ${upgradeProgramPath}`
          )
        }
        ...
        if (programKeypair) {
          const keypairJson = JSON.parse(
            fs.readFileSync(programKeypair, "utf8")
          )
          programKeypairData = Keypair.fromSecretKey(
