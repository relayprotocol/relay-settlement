/**
 * Lit Action: Ethereum signer.
 *
 * Runs inside Lit's TEE. Derives the Ethereum address from the PKP private key
...
interface EthereumActionParams {
...
interface EthereumWalletResult {
  vmType: "ethereum-vm";
  /** EIP-55-checksummed Ethereum address derived from the PKP. */
  address: string;
}

/** Result returned for `action="sign"`. Carries one signature per attested hash. */
interface EthereumSignResult extends EthereumWalletResult {
...
/** Derive the Ethereum VM secp256k1 private key as hex from the PKP private key. */
async function deriveEthereumPrivateKeyHex(pkpPrivateKeyHex: string): Promise<string> {
  return bytesToHex(await deriveKey(pkpPrivateKeyHex, "ethereum-vm"));
}

/** Derive an Ethereum address from a hex-encoded secp256k1 private key. */
function ethereumAddressFromPrivateKey(privateKeyHex: string): string {
...
/** Lit Action entrypoint for wallet lookup and attestation-gated Ethereum signing. */
export async function main({
  pkpId,
  action,
  withdrawRequest,
  attestation,
}: EthereumActionParams): Promise<EthereumWalletResult | EthereumSignResult> {
  const pkpPrivateKey = await Lit.Actions.getPrivateKey({ pkpId });
  const ethereumPrivateKey = await deriveEthereumPrivateKeyHex(pkpPrivateKey);

  if (action === "wallet") {
    return {
      vmType: "ethereum-vm",
      address: ethereumAddressFromPrivateKey(ethereumPrivateKey),
    };
  }
  ...
  if (action === "sign") {
    ...
    return {
      ...
      results: hashesToSign.map((hash) => ({
        ...
        signature: signDigest(hash, ethereumPrivateKey),
      })),
    };
  }
