...
async function billingPost<T>(path: string, apiKey: string, body: object): Promise<T> {
...
<head>
  <meta charset="utf-8">
  <title>Lit Allocator — Top Up ($${dollars})</title>
...
<body>
  <h1>Top up Lit Allocator account</h1>
...
<head>
  <meta charset="utf-8">
  <title>Lit Allocator — ${ok ? "Payment received" : "Payment status"}</title>
  <style>
...
function serveStripeForm(
  ...
  return new Promise((resolveServer, rejectServer) => {
    ...
    const done = new Promise<{ status: string; paymentIntentId: string }>((res, rej) => {
      resolveDone = res;
      rejectDone = rej;
    });
...
async function main() {
  ...
  if (!Number.isFinite(amountCents) || amountCents < MIN_AMOUNT_CENTS) {
    console.error(`--amount-cents must be an integer >= ${MIN_AMOUNT_CENTS} (= $5.00 minimum)`);
    process.exit(1);
  }
  ...
  const stripeConfig = await billingGet<StripeConfigResponse>("/core/v1/billing/stripe_config");
  console.log(`   ✓ publishable_key=${stripeConfig.publishable_key.slice(0, 12)}…`);
  ...
  console.log();
  console.log("   👉 Opening your browser. If it doesn't open, visit the URL above manually.");
  console.log("      Connect a wallet (USDC/ETH/SOL/USDP supported via Stripe crypto on-ramp),");
  console.log("      approve the on-chain transaction, and Stripe will redirect back here once");
  ...
  console.log(`   ✓ ${balance.balance_display} (balance_cents=${balance.balance_cents})`);
}
