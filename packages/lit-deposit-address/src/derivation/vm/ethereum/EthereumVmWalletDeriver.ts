...
import {
  encodeEthereumAddressToHex,
  normalizeEthereumAddressHex,
} from "../../../common/address/ethereum.js";
...
> {
  ...
  override verifyTransactions(
    ...
    if (normalizeEthereumAddressHex(trigger.input.currency) === ZERO_ADDRESS) {
  ...
  private verifyNativeDeposit(
    ...
    const expectedTo = normalizeEthereumAddressHex(attestation.inputDepository);
    const actualTo = parsed.to ? encodeEthereumAddressToHex(parsed.to).toLowerCase() : undefined;
    ...
    const depositorHex = encodeEthereumAddressToHex(depositor).toLowerCase();
    const expectedDepositor = normalizeEthereumAddressHex(trigger.derivationFields.depositor);
  ...
  private verifyErc20Deposit(
    ...
    const expectedTokenTo = normalizeEthereumAddressHex(trigger.input.currency);
    const actualTokenTo = approveTx.to
      ? encodeEthereumAddressToHex(approveTx.to).toLowerCase()
      : undefined;
    ...
    const approveSpenderHex = encodeEthereumAddressToHex(approveSpender).toLowerCase();
    const expectedSpender = normalizeEthereumAddressHex(attestation.inputDepository);
    ...
    const expectedDepositTo = normalizeEthereumAddressHex(attestation.inputDepository);
    const actualDepositTo = depositTx.to
      ? encodeEthereumAddressToHex(depositTx.to).toLowerCase()
      : undefined;
    ...
    const erc20DepositorHex = encodeEthereumAddressToHex(depositor).toLowerCase();
    const expectedErc20Depositor = normalizeEthereumAddressHex(trigger.derivationFields.depositor);
    ...
    const tokenHex = encodeEthereumAddressToHex(token).toLowerCase();
    const expectedToken = normalizeEthereumAddressHex(trigger.input.currency);
