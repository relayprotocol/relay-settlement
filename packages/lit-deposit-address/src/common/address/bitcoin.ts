...
/** Round-trip SDK-encoded bitcoin-vm address hex through the codec. */
export function normalizeBitcoinAddressHex(encodedHex: string): string {
  return encodeBitcoinAddressToHex(
    decodeBitcoinAddress(hexToBytes(encodedHex, "encoded address")),
  ).toLowerCase();
}
