...
async function main() {
  ...
  if (adminAddressArg && adminPrivateKey) {
    console.error(
      `Pass only one of --admin-address or --admin-private-key, not both.\n\n${USAGE}`
    )
    process.exit(1)
  }
  ...
  if (sim.ok) {
    ...
    console.log(
      "  This is an inconsistency between the contract and this script's assumptions."
    )
    process.exit(2)
  }
  ...
  if (isPositive) {
    ...
    console.log(
      `  Contract confirmed via eth_call (revert: "${decoded.message}")`
    )
    return
  }
  ...
  switch (decoded.name) {
    case "NoAccountAccess":
      console.log(
        `  msg.sender is not registered as admin for this apiKeyHash.`
      )
      break
    case "AccountDoesNotExist":
      console.log(
        `  No account found on-chain for apiKeyHash ${apiKeyHash.toString(16)}.`
      )
      console.log(
        `  Either --account-api-key is wrong or the account was never created.`
      )
      break
    case "InvalidRequest":
      console.log(
        `  Contract rejected the probe: "${decoded.message ?? "(no message)"}"`
      )
      if (decoded.message?.toLowerCase().includes("not chainsecured")) {
        console.log(
          `  Account is still in API mode — run convert-to-chain-secured first.`
        )
      }
      break
