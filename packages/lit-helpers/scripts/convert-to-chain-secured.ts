...
const CONVERT_URL =
  "https://api.chipotle.litprotocol.com/core/v1/convert_to_chain_secured_account"
 ...
 */
async function isLookupHashFree(
  rpcUrl: string,
  lookupHash: bigint
): Promise<boolean> {
  ...
  const res = await fetch(rpcUrl, {
    ...
    body: JSON.stringify({
      ...
      params: [
        {
          from: dummyNewAdmin,
          to: DEFAULT_ACCOUNT_CONFIG_ADDRESS,
          data: toHex(calldata),
        },
        "latest",
      ],
  ...
  if (!body.error) {
    throw new Error(
      `isLookupHashFree: probe unexpectedly succeeded with result ${body.result}`
    )
  }
...
async function main() {
  ...
  if (!free) {
    ...
    console.error(
      `  allApiKeyHashesToMaster[${formatHash(newAdminAddressHash)}] is already set,`
    )
    console.error(
      `  which means this wallet has been admin of some account before.`
    )
    console.error(
      `  The contract never frees these slots, so a fresh wallet is required.`
    )
  ...
  if (!res.ok) {
    const explained = explainContractError(
      text,
      apiKeyHash,
      newAdminAddressHash
    )
  ...
  console.log(
    `  Admin writes (group/action/PKP/usage-key mutations) now require signing with the`
  )
  console.log(`  new admin wallet ${newAdminAddress}.`)
}
