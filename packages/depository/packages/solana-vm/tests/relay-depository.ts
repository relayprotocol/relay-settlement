...
describe("Relay Depository", () => {
  ...
  it("Should successfully initialize with correct owner", async function () {
    ...
    assert.equal(relayDepositoryAccount.vaultBump, vaultBump)
    assert.deepEqual(
      new Uint8Array(relayDepositoryAccount.domainSeparator),
      domainSeparator
    )
  })
  ...
  it("Deposit token2022 with transfer fee - verify amount excludes fee", async () => {
    ...
    const userBalanceBefore =
      await provider.connection.getTokenAccountBalance(userFeeTokenAccount)
    ...
    // Get vault's balance
    const vaultBalanceAfter =
      await provider.connection.getTokenAccountBalance(vaultFeeTokenAccount)
  ...
  it("Execute native transfer with allocator signature", async () => {
    ...
    const usedRequestState = await program.account.usedRequest.fetch(requestPDA)
  ...
  it("Should fail with message mismatch (signed different request)", async () => {
    const transferAmount = LAMPORTS_PER_SOL / 10
    const sharedNonce = new anchor.BN(
  ...
  it("Should fail cross-chain replay attack with domain separator", async () => {
    ...
    const mainnetSignature = nacl.sign.detached(
      mainnetMessageHash,
      allocator.secretKey
    )

    const testnetMessageHash = hashRequest(testnetRequest)
    const testnetSignature = nacl.sign.detached(
      testnetMessageHash,
      allocator.secretKey
    )
    ...
    const mainnetUsedRequestState =
      await program.account.usedRequest.fetch(mainnetRequestPDA)
