...
const compiledModuleUsdc = {
  ...
  digest: [
    201, 60, 152, 72, 135, 186, 20, 153, 47, 119, 37, 161, 105, 93, 32, 172,
    208, 77, 109, 22, 21, 211, 148, 80, 90, 227, 126, 254, 90, 98, 250, 73,
  ],
}
...
describe("Relay Escrow", () => {
  ...
  before(async () => {
    ...
    await Promise.all(
      [
        ...
        requestSuiFromFaucetV0({ host: getFaucetHost(networkType), recipient })
      )
    )

    const packageData = await publishPackage(
      __dirname + "/../relay-escrow",
      CHAIN_ID
    )
  ...
  it("should fail execute transfer when allocator_pubkey is not set (EInvalidPublicKey)", async () => {
    try {
      ...
      const [coin] = depositTx.splitCoins(coins.data[0].coinObjectId, [
      ...
      const serializedData =
        TransferRequestStruct.serialize(transferRequest).toBytes()
  ...
  it("should set new allocator successfully", async () => {
    try {
      ...
      getTx.moveCall({
        ...
        arguments: [getTx.object(ESCROW_ID)],
      })
      ...
      const oldAllocatorInfo = AllocatorInfoStruct.parse(
      ...
      verifyTx.moveCall({
        target: `${PACKAGE_ID}::escrow::get_allocator`,
        arguments: [verifyTx.object(ESCROW_ID)],
      })
      ...
      const allocatorInfo = AllocatorInfoStruct.parse(
        new Uint8Array(verifyResponse.results![0].returnValues![0][0])
      )
      ...
      const allocatorChangedEvent = events?.find((event) =>
        event.type.includes("AllocatorChangedEvent")
      )
  ...
  it("should execute transfer successfully with valid signature", async () => {
    try {
      ...
      const bobBalanceBefore = await getCoinBalance(
        bob.toSuiAddress(),
        "0x2::sui::SUI"
      )
      ...
      const bobBalanceAfter = await getCoinBalance(
        bob.toSuiAddress(),
        "0x2::sui::SUI"
      )
      ...
      const transferEvent = events?.find((event) =>
        event.type.includes("TransferExecutedEvent")
      )
  ...
  it("should execute USDC transfer successfully with valid signature", async () => {
    try {
      ...
      expect(transferEvent?.parsedJson?.recipient).to.equal(recipient)
      expect(BigInt(transferEvent?.parsedJson?.amount) === amount).to.be.true
      expect(transferEvent?.parsedJson?.coin_type.name).to.equal(
        normalizeType(USDC_COIN_TYPE)
      )
    } catch (error) {
  ...
  it("should execute multiple transfers in a single transaction", async () => {
    try {
      ...
      const depositTx = new Transaction()
      const [coin] = depositTx.splitCoins(primaryCoin.coinObjectId, [
      ...
      const bobInitialBalance = await getCoinBalance(
        bob.toSuiAddress(),
        "0x2::sui::SUI"
      )
      ...
      const signedTransfers = await Promise.all(
      ...
      const gasCoin = updatedCoins.data.find(
        (coin) => BigInt(coin.balance) >= 150000000n
      )
      ...
      const bobFinalBalance = await getCoinBalance(
        bob.toSuiAddress(),
        "0x2::sui::SUI"
      )
      ...
      expect(initialEscrowBalance - escrowFinalBalance).to.equal(
        totalWithdrawAmount
      )
      ...
      transferEvents?.forEach((event, index) => {
        ...
        expect(BigInt(event.parsedJson?.amount)).to.equal(
      ...
      console.error(
        "Multiple transfers in single transaction test failed:",
        error
      )
      throw error
    }
  ...
  async function publishPackage(packagePath: string, chainId: string) {
    ...
    const allocatorCapId = objects.find(
      (c) => c.type === "escrow::AllocatorCap"
    ).id!
    const executedRequestId = objects.find(
      (c) => c.type === "escrow::ExecutedRequests"
    ).id!
  ...
  async function deployCoin() {
    ...
    const packageId = objects.find((c) => c.type === "package").id!
    const treasuryCapId = objects.find((c) =>
      c.typeRaw.includes("coin::TreasuryCap")
    ).id!
    return {
      packageId,
      treasuryCapId,
