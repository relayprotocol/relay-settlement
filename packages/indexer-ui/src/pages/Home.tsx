...
export default function Home() {
  ...
  const [auditReport, setAuditReport] = useState<IndexerAuditReport | null>(
    null
  )
  ...
  const formatAuditKind = (value: string) => value.replace("-", " ")

  const formatClassification = (value: string) => value.replaceAll("_", " ")

  const formatBlockRange = (fromBlock: number, toBlock: number) =>
    `${fromBlock.toLocaleString("en-US")} - ${toBlock.toLocaleString("en-US")}`
  ...
  const auditFindingExamples = useMemo(
    () =>
      (auditReport?.findings ?? [])
        .filter((finding) => finding.classification === "confirmed")
        .slice(0, 5),
    [auditReport]
  )
  ...
  const loadAuditReport = async () => {
    try {
      const data = await fetchIndexerAuditLatest()
      setAuditReport(data)
      setAuditError("")
    } catch (err) {
      setAuditReport(null)
      setAuditError(
        err instanceof Error ? err.message : "Failed to load indexer audit"
      )
    }
  }
    ...
    <div className="app">
      ...
      <section className="panel audit-panel">
        <div className="panel-header">
          ...
          {auditReport ? (
            <span
              className={`status-pill ${auditReport.ok ? "succeeded" : "failed"}`}
            >
              {auditReport.ok ? "healthy" : "needs review"}
            </span>
          ) : (
            <span className="status-pill">unknown</span>
          )}
        </div>
        {auditError ? <div className="empty">{auditError}</div> : null}
        {!auditReport && !auditError ? (
          <div className="empty">No audit report available yet.</div>
        ) : null}
        {auditReport ? (
          <>
            <div className="stat audit-stats">
              <div>
                <p className="label">Checked rows/logs</p>
                <p className="value">
                  {auditTotals.checked.toLocaleString("en-US")}
                </p>
              </div>
              <div>
                <p className="label">Confirmed findings</p>
                <p className="value">
                  {auditTotals.confirmed.toLocaleString("en-US")}
                </p>
              </div>
              <div>
            ...
            <div className="table audit-health-table">
              ...
              {auditReport.health.kinds.map((kind) => (
                <div key={kind.kind} className="table-row">
                  <span>{formatAuditKind(kind.kind)}</span>
                  <span>
                    <span
                      className={`status-pill ${kind.ok ? "succeeded" : "failed"}`}
                    >
                      {kind.status}
                    </span>
                    {kind.reason ? (
                      <span className="muted"> {kind.reason}</span>
                    ) : null}
                  </span>
                  <span>{kind.confirmedCount.toLocaleString("en-US")}</span>
                  <span>
                    {kind.completedAt
                      ? new Date(kind.completedAt).toLocaleString("en-US", {
                          hour12: false,
                        })
                      : "-"}
                  </span>
            ...
            {auditReport.suggestedReplayRanges.length ? (
              <div className="table audit-ranges-table">
                <div className="table-row header">
                  <span>Suggested replay range</span>
                  <span>Reason</span>
                  <span>Findings</span>
                </div>
                {auditReport.suggestedReplayRanges.slice(0, 5).map((range) => (
                  <div
                    key={`${range.fromBlock}-${range.toBlock}-${range.reason}`}
                    className="table-row"
                  >
                    <span>
                      <Link className="token-link" to="/admin/replay">
                        {formatBlockRange(range.fromBlock, range.toBlock)}
                      </Link>
                    </span>
                    <span>{formatAuditKind(range.reason)}</span>
                    <span>{range.findingCount.toLocaleString("en-US")}</span>
                  </div>
                ))}
              </div>
            ) : null}

            {auditFindingExamples.length ? (
              <div className="table audit-findings-table">
                <div className="table-row header">
                  <span>Finding</span>
                  <span>Token</span>
                    ...
                    <span>
                      {finding.tokenId ? (
                        <Link
                          ...
                          to={tokenPath(finding.tokenId)}
                        >
                          {displayTokenName(finding.tokenName)}
                        </Link>
                      ) : (
                        "-"
                      )}
