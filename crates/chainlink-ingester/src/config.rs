...
const DEFAULT_ADDRESS: &str = "127.0.0.1:9801";
...
pub fn get() -> Result<Config> {
    Ok(Config {
        ...
        credentials: Credentials::new(
            ...
            required_env("CHAINLINK_DATA_STREAMS_API_SECRET")?,
        ),
        feed_ids: parse_feed_ids(&required_env("CHAINLINK_FEED_IDS")?)?,
...
fn parse_max_age_sec() -> Result<u64> {
    let Some(raw) = optional_env("CHAINLINK_MAX_AGE_SEC") else {
        return Ok(DEFAULT_MAX_AGE_SEC);
    };
    let secs = raw
        .parse::<u64>()
        .context("CHAINLINK_MAX_AGE_SEC must be a positive integer")?;
    if secs == 0 {
        return Err(anyhow!("CHAINLINK_MAX_AGE_SEC must be greater than zero"));
    }
    Ok(secs)
}
...
fn parse_feed_ids(raw: &str) -> Result<Vec<B256>> {
    ...
    for (idx, item) in raw.split(',').enumerate() {
        ...
        let bytes = hex::decode(raw_hex)
            .with_context(|| format!("CHAINLINK_FEED_IDS[{idx}] is not valid hex: {trimmed}"))?;
        let arr: [u8; 32] = bytes.try_into().map_err(|got: Vec<u8>| {
            anyhow!(
                "CHAINLINK_FEED_IDS[{idx}] must be exactly 32 bytes, got {} bytes",
                got.len()
            )
        })?;
    ...
    if feed_ids.is_empty() {
        return Err(anyhow!(
            "CHAINLINK_FEED_IDS must contain at least one feed id"
        ));
    }
    Ok(feed_ids)
}
