...
each feed, and serves those reports to a single sequencer subscriber over an IPC
connection (TCP by default). Reports are decoded only to verify them (see below)
...
- `INGESTER_SOCKET_ADDRESS` - TCP listen address, defaults to `127.0.0.1:9801` (used when transport is `tcp`).
- `INGESTER_SOCKET_PATH` - Unix socket path, defaults to `/run/relay/chainlink.sock` (used when transport is `unix`).
